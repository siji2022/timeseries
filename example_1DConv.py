...
seasonality_freq=50
LEN=400
SEQ=50 # 50 dramatically improves the performance; maybe related to the seasonality_freq
TEST_SIZE=0.2
USE_DIFF=False
...
# if p-value(e.g 0.01)<0.05, we can reject the null hypothesis; hence the data is non-stationary
...
class Conv1DModel(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1=nn.Conv1d(1,channel,kernel_size=25)
        self.conv2=nn.Conv1d(self.conv1.out_channels,channel,kernel_size=25)
    ...
    def forward(self, x):
        ...
        # print(x.shape) # [batch,channel,3] ks=SEQ-2
        x=torch.max_pool1d(x,2).squeeze()
        # print(x.shape) # [batch,channel]
