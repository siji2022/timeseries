...
def test(model,test_loader,crit):
    ...
    with torch.no_grad():
        pred=[]
        gt=[]
        x=test_tensor[0][0].to(device) #2, 1
        # print(x.shape)
...
class Conv1DModel(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.fc1=nn.Linear(SEQ,channel)
...
optimizer = torch.optim.Adam(model.parameters(), weight_decay=1e-3,lr=0.005) 
