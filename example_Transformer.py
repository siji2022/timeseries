...
class TransModel(torch.nn.Module):
    def __init__(self,seq,ffd,num_layers,heads,dp) -> None:
        super().__init__()
        # using transformer
        self.trans1=nn.TransformerEncoderLayer(d_model=seq, nhead=heads,activation=torch.tanh,
                                               dim_feedforward=ffd,dropout=dp)
        self.transform=nn.TransformerEncoder(self.trans1,num_layers=num_layers)
        # self.fc1=nn.Linear(SEQ,hs)
    ...
    def forward(self, x):
        ...
        x=self.transform(x) #16,1,SEQ
        # print(x.shape)
        # x=self.dropout(x)
        # use all the hidden states
        x=x[:,:,-1]
        # x=F.relu(self.fc1(x))#16,hs 
        # x=self.dropout(x)
        # x=self.fc2(x)
        # print(x.shape)
        return x
...
for dp in [0]:
    idx+=1
    model=TransModel(seq=SEQ,ffd=128,num_layers=4,heads=1,dp=0.1).to(device)
